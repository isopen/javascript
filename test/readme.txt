...
поля: id(int auto inc), user_id(bigint), chat_id(bigint), name, created(timestamp with timezone), updated(timestamp with timezone)
...
4) При клике на чат(у нас уже есть в локальном хранилище chat_id. допустим chat_id = 76757473) следоветельно запрашиваем сообщения чата по таблице messanger_messages
